...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Enhanced routing logic to support standard paths and hash paths (for static hosting/PWAs)
    const checkAdminRoute = () => {
        const path = window.location.pathname;
        const hash = window.location.hash;
        
        // Checks for: domain.com/admin OR domain.com/#/admin OR domain.com/#admin
    ...
    // Listen for hash changes to allow dynamic switching without reload
  ...
  const handleBackToWelcome = () => {
      // Clear hash if in admin mode to prevent staying in admin on back
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-indigo-50 to-purple-50 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950 animate-gradient-xy text-soft-text dark:text-slate-200 font-sans selection:bg-sky-200 dark:selection:bg-slate-700 transition-colors duration-300">
