...
import { IconPlus, IconTrash, IconArrowLeft, IconBook, IconHeadphones, IconMusic, IconMessage, IconSparkles, IconLock, IconLogin } from './Icons';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onBack }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
      ...
      // In production, use environment variables or backend auth.
      // Update: Accepting 'kzi207' as valid admin password.
      if (passwordInput === 'admin' || passwordInput === '123456' || passwordInput === 'kzi207') {
  ...
  if (!isAuthenticated) {
      return (
          <div className="min-h-screen bg-sky-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 animate-fade-in-up">
              <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl max-w-sm w-full border border-slate-100 dark:border-slate-800">
                  <div className="flex justify-center mb-6">
                      <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-500">
                          <IconLock className="w-8 h-8" />
                      </div>
                  </div>
                  <h2 className="text-2xl font-bold text-center text-slate-800 dark:text-slate-100 mb-2">Admin Access</h2>
                  <p className="text-sm text-center text-slate-500 dark:text-slate-400 mb-6">Vui lòng nhập mật khẩu quản trị viên.</p>
                  ...
                  <form onSubmit={handleLogin} className="space-y-4">
                      <input 
                          ...
                          className="w-full p-4 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-sky-500 text-center tracking-widest text-lg font-bold"
                          placeholder="••••••"
                          autoFocus
                      />
                      <button 
                          type="submit" 
                          className="w-full py-3 bg-slate-800 dark:bg-slate-700 text-white rounded-xl font-bold hover:bg-slate-900 dark:hover:bg-slate-600 transition flex items-center justify-center gap-2"
                      >
                          <IconLogin className="w-5 h-5" /> Truy cập
                      </button>
                  </form>
                  
                  <button onClick={onBack} className="w-full mt-4 py-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-sm font-medium transition">
                      Quay lại trang chủ
                  </button>
              </div>
          </div>
