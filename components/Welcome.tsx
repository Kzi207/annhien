...
const Fireflies = () => {
    const fireflies = useMemo(() => {
        return Array.from({ length: 20 }).map((_, i) => ({
            ...
            moveX: `${-50 + Math.random() * 100}px`,
            moveY: `${-50 + Math.random() * 100}px`,
        }));
    }, []);
...
const ShootingStars = () => {
    return (
         <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
             <div className="shooting-star" style={{'--top': '10%', '--left': '80%', '--delay': '2s'} as any}></div>
             <div className="shooting-star" style={{'--top': '20%', '--left': '90%', '--delay': '8s'} as any}></div>
             <div className="shooting-star" style={{'--top': '5%', '--left': '50%', '--delay': '15s'} as any}></div>
         </div>
    )
}
...
const Welcome: React.FC<WelcomeProps> = ({ 
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[100dvh] p-6 text-center animate-fade-in-up relative overflow-hidden transition-colors duration-500 bg-sky-50 dark:bg-slate-950">
      ...
      {/* --- NEW DYNAMIC EFFECTS --- */}
      <div className="opacity-0 dark:opacity-60 transition-opacity duration-1000">
         <StarField />
      </div>
      <Fireflies />
      <ShootingStars />
      ...
      <div className="glass-card relative p-8 sm:p-14 rounded-[32px] max-w-sm sm:max-w-md w-full z-10 transition-all duration-300 group hover:shadow-glow border-2 border-white/60 dark:border-white/5">
        ...
        {/* Text */}
        <h1 className="text-4xl sm:text-5xl font-serif font-bold text-slate-800 dark:text-slate-100 mb-3 tracking-tight relative">
            An Nhiên
        </h1>
        ...
        <div className="mt-6 pt-6 border-t border-dashed border-slate-200 dark:border-slate-700 h-16 flex items-center justify-center">
             <p className="text-xs text-slate-400 dark:text-slate-500 font-serif italic text-center">
              "<TypewriterText text="Mong bạn những ngày sau này, bão giông dừng lại sau cánh cửa." delay={500} />"
            </p>
        </div>
      </div>
